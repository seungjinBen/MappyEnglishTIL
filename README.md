...
---
* 배포를 위한 최적화를 진행한다.
* 구글지도 전역 로딩처리: 라우팅을 이동해도 스크립트가 브라우저에 캐시된 상태로 유지
  - 자식 컴포넌트에서는 로딩 건너뛰고 true처리
* SVG 생성 함수를 컴포넌트 밖으로 처리
  - 기존에는 컴포넌트가 리렌더링 될 때마다(지도 이동, 클릭 등)이 함수가 계속 새로 만들어지고 있었음.
*  아이콘 URL 캐싱(Memoization)
  - 마커가 100개라도, 'A'타입 아이콘은 딱 한 번만 문자열을 생성한다.
* Firebase Storage 서버와 미리 연결 -> index.html의 헤더에 코드 추가
